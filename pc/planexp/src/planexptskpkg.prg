...
-- 01b,15.10.15,pluc    Envoi tol max à cubeiq si pas de rajout de palette en testant le volume max.
-- 01a,31.07.13,tcho    Création
-- 00a,31.07.13,GENPRG  version 2.13
-- -------------------------------------------------------------------------
--
...
BEGIN
    ...
    if g_cfg_pal.mode_calc_seuil_vol='90' then  -- SUO : UP cible
        ...
        if g_nb_pal=0 or g_nb_pal*g_cfg_pal.vol_max*g_taux_remp<p_vol then -- $MOD 01c : si volume à palettiser > volume max alors rajouter une palette
            g_nb_pal:=g_nb_pal+1;
        ELSE
            -- $MOD,pluc Envoi tol max à cubeiq si pas de rajout de palette en testant le volume max.
            g_tol_max:=g_cfg_pal.tol_max;
        end if;

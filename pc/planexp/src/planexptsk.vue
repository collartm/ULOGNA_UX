...
-- DESCRIPTION :
-- -------------
-- Ces vues sont utilisées dans pc_plan_exp_tsk_pkg
--
-- HISTORIQUE DES MODIFICATIONS :
-- -------------------------------------------------------------------------
-- Ver,Date    ,Auteur Description
-- -------------------------------------------------------------------------
-- 02a,16.06.14   sépcif systeme U
...
CREATE OR REPLACE VIEW V_PEXPTSK_UEE AS
    select  c.no_dpt,
        ...
        l.cle_rgp_pal_2,
         decode(l.cle_rgp_pal_2,NULL,NULL,L.cle_rgp_pal_2,pc_mod_pal_pkg.pc_bas_sgn_pal (   -- signature dégradée
                    l.mode_pal_1,
                    l.typ_pal_1       ,
                    c.cod_usn       ,
                    c.cod_soc       ,
      ...
      where c.no_com=l.no_com
        ...
        AND l.typ_pal_1='HETER' -- uniquement hétérogène pour le moment
/
    ...
    select  *
      ...
      where lst_fct_lock_lig IS NULL
        ...
        AND ( su_bas_rch_cle_atv_pss_2 (cod_pss_afc, 'POR', 'MODE_CAL_COORD_UEE') = '1'
               AND cod_cli NOT IN (SELECT cod_cli         -- attendre la fin de la purge sur le séquenceur pour ne pas générer n petites UP
...
CREATE OR REPLACE VIEW V_PEXPTSK_CFG_PAL AS
    select c.*,
           ...
           c.vol_std*(nvl(c.SEUIL_MULTI,0)/100) vol_seuil    -- volume pour dégradation solution
